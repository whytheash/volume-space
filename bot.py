...
QUESTIONS = {
    ...
    TestStates.Q4: (
        ...
        ["A. я за любые идеи, фрихенд - вперед!", "B. предложу альтернативу из свободных эскизов", "C. перенесу сеанс, чтобы успеть подготовить эскиз"]
    ),
    TestStates.Q5: (
    ...
    TestStates.Q7: (
        ...
        ["A. не пойду, мне еще на завтра эскиз готовить", "B. залечу на пару часов и домой", "C. перенесу сеанс и буду тусить до утра"]
    ),
    TestStates.Q8: (
    ...
    TestStates.Q9: (
        ...
        ["A. обозлюсь, но не подам вида", "B. «освободилось окошко, отдам со скидкой»", "C. не буду с ним работать, со мной так нельзя"]
    ),
    TestStates.Q10: (
...
"мы проделали большой путь вместе с командой, чтобы в итоге этот тест вышел в свет. надеемся, что он тебе понравится, а может даже вдохновит на что-то новое! \nрезультаты могут помочь понять в каком направлении развиваться, даже если сейчас ты новичок или вовсе не знаком с индустрией. \n\n" \
"хотим напомнить: этот тест создан исключительно в целях развлечения и самопознания. он примерно определяет тип по твоим действиям в тату-индустрии и позволяет посмотреть на свое искусство по-новому,\nно не является профессиональной оценкой или руководством к действию. \n\n" \
"поехали!"
